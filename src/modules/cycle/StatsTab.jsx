...
const MOOD_COLORS  = ['#E8829A', '#C4AFA8', 'rgba(59,51,48,0.4)', '#8FA58C', '#E8829A']
const MOOD_LABELS  = ['Low', 'Anxious', 'Neutral', 'Good', 'Great']

// ── SVG bar chart ─────────────────────────────────────────────────────────────
function BarChart({ values, barColor, labels }) {
  if (!values || values.length === 0) {
    return <p className="font-garamond text-brown/30 text-sm text-center py-6">Not enough data yet.</p>
  }
  const W       = 280
  const H       = 90
...
function StatTile({ icon, value, label }) {
  return (
    <div
      ...
      style={{ background: 'rgba(232,130,154,0.07)', border: '1px solid rgba(232,130,154,0.15)' }}
    >
      <span className="text-lg mb-1">{icon}</span>
      <span className="font-cinzel text-rose text-xl leading-none">{value ?? '—'}</span>
      <span className="font-garamond text-brown/45 text-xs mt-1 text-center leading-tight">{label}</span>
    </div>
  )
}
...
function Shimmer({ className = '' }) {
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(90deg, rgba(232,130,154,0.06) 25%, rgba(232,130,154,0.12) 50%, rgba(232,130,154,0.06) 75%)',
  ...
  return (
    <div className="space-y-5 pb-4">
      ...
      >
        ...
        <BarChart values={stats.cycleLengths} barColor="rgba(232,130,154,0.75)" />
      </div>
              ...
              <div key={name}>
                ...
                <div className="h-1.5 rounded-full" style={{ background: 'rgba(59,51,48,0.08)' }}>
                  <div
                    ...
                    style={{ width: `${pct}%`, background: 'rgba(232,130,154,0.65)' }}
                  />
                </div>
              </div>
            ))}
      ...
      {moodTotal > 0 && (
        ...
        >
          <p className="font-cinzel text-brown/50 text-[10px] tracking-widest uppercase mb-3">Mood — Last 30 Days</p>
              ...
              <div key={c.id ?? i}>
                ...
                >
                  <div>
                    ...
                    <span className="font-garamond text-brown/55 text-sm">
                  ...
                  <div className="flex items-center gap-3">
                    {len && (
                      <span className="font-garamond text-rose/70 text-xs">{len}d</span>
                    )}
                    <span className="text-brown/30 text-xs">{isOpen ? '▾' : '›'}</span>
                  </div>
                </button>

                {isOpen && (
                  <div className="px-4 pb-3" style={{ background: 'rgba(232,130,154,0.04)' }}>
                    ...
                    {cycleSym.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-1">
                        {Array.from(new Set(cycleSym.flatMap(s => {
                          ...
                          <span key={sym} className="font-garamond text-brown/35 text-[10px] border border-ivory/10 px-1.5 py-0.5 rounded-full">
                            {sym}
                          </span>
                        ))}
                      </div>
      ...
      <div
        ...
        style={{ background: 'rgba(232,130,154,0.07)', border: '1px solid rgba(232,130,154,0.28)' }}
      >
        ...
        {insightLoad ? (
          ...
          <>
            {insight.headline && (
              <p className="font-cinzel text-brown/80 text-sm mb-2">{insight.headline}</p>
            )}
            <p className="font-garamond italic text-brown/65 text-sm leading-relaxed mb-3">
              {insight.body}
            </p>
            {insight.tip && (
              <p className="font-garamond text-brown/50 text-xs border-t border-ivory/10 pt-3">
                ✦ {insight.tip}
              </p>
            )}
            <button
              onClick={fetchInsight}
              className="font-garamond text-rose/60 text-xs mt-3 hover:text-rose transition-colors"
            >
              Refresh insight
            </button>
          </>
          ...
          <div className="text-center py-2">
            <p className="font-garamond text-brown/35 text-sm mb-3">
              {phaseData?.phase ? 'Unable to load insight.' : 'Set up your cycle to get personalized insights.'}
            </p>
            {phaseData?.phase && (
              <button
                onClick={fetchInsight}
                className="font-cinzel text-rose/60 text-[10px] tracking-widest uppercase border border-rose/25 px-4 py-2 rounded-xl hover:bg-rose/10 transition-colors"
              >
                Try Again
              </button>
            )}
