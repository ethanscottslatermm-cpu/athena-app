...
export default function MuscleBottomSheet({
  ...
  const relevantSessions = useMemo(() => {
    ...
    return sessionHistory.filter(s =>
      (s.muscleGroups ?? []).includes(pairKey)
    )
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Drag handle */}
        <div style={{ width: 40, height: 4, borderRadius: 2, background: 'rgba(201,168,108,0.2)', margin: '12px auto 0' }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '1.25rem', marginBottom: '0.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <p style={{ fontFamily: fontSerif, fontSize: 22, color: linen, margin: 0, lineHeight: 1.1 }}>{name}</p>
              <p style={{ fontFamily: fontSans, fontSize: 12, color: mutedText, margin: '2px 0 0', letterSpacing: '0.04em' }}>{anatom}</p>
            </div>
          </div>
          ...
          {isPrimary && currentPhase && (
            <span style={{
              ...
              border:      `1px solid ${currentPhase.phaseColor}50`,
              borderRadius: 20,
              padding:     '3px 10px',
              background:  `${currentPhase.phaseColor}15`,
            }}>
              ✦ Recommended
            </span>
          )}
          ...
          {isAvoid && currentPhase && (
            <span style={{
              ...
              textTransform: 'uppercase',
              color:       '#C4606A',
              border:      '1px solid rgba(196,96,106,0.35)',
              borderRadius: 20,
              padding:     '3px 10px',
              background:  'rgba(196,96,106,0.08)',
            }}>
              Rest Today
            </span>
          )}
        ...
        {(isPrimary || isSecondary) && currentPhase && phaseRec && (
          <div style={{
            background:   `${currentPhase.phaseColor}10`,
            border:       `1px solid ${currentPhase.phaseColor}30`,
            borderRadius: 10,
            padding:      '10px 14px',
            marginBottom: '1.25rem',
          }}>
            <p style={{ fontFamily: fontSerif, fontSize: 13, color: currentPhase.phaseColor, margin: 0, lineHeight: 1.5 }}>
        ...
        {sessionCards.length > 0 ? (
          <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 8, marginBottom: '1rem' }}>
            {sessionCards.map(s => (
              ...
              >
                <p style={{ fontFamily: fontSerif, fontSize: 14, color: linen, margin: '0 0 4px', lineHeight: 1.3 }}>
                  {s.title ?? s.name}
                </p>
                <p style={{ fontFamily: fontSans, fontSize: 10, color: mutedText, margin: 0 }}>
                  {s.duration_min ?? s.duration} min
                </p>
                {(s.muscleGroups ?? []).slice(0, 2).map(m => (
                  <span key={m} style={{
                    ...
                    color:        MUSCLE_COLORS[m] ?? mutedText,
                    border:       `1px solid ${MUSCLE_COLORS[m] ?? mutedText}40`,
                    borderRadius: 10,
                    padding:      '1px 6px',
                  }}>
                    {MUSCLE_NAMES[m]}
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: '1.25rem' }}>
          <div style={{ flex: 1, background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '10px 12px' }}>
            ...
            <p style={{ fontFamily: fontSerif, fontSize: 14, color: linen, margin: 0 }}>{formatRelativeDate(lastTrained)}</p>
          </div>
          <div style={{ flex: 1, background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '10px 12px' }}>
            ...
            <p style={{ fontFamily: fontSerif, fontSize: 14, color: linen, margin: '0 0 6px' }}>{thisMonthCount} / {MONTHLY_GOAL} sessions</p>
