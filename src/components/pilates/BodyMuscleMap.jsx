...
// Everything is one SVG — the PNG is rendered as <image> so the recolor filter
// uses the PNG's own alpha channel, painting only the silhouette shape.
...
export default function BodyMuscleMap({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
      ...
      {/*
        Single SVG renders both the silhouette and muscle overlays.
        <image> uses the PNG's own alpha channel, so feFlood+feComposite
        paints only the silhouette shape — transparent pixels stay transparent.
      */}
      <svg
        viewBox={`0 0 ${VW} ${VH}`}
      ...
      >
        <defs>
          {/* Recolor silhouette: SourceAlpha masks the flood to the exact PNG shape */}
          <filter id="silhouette-color" x="0" y="0" width="100%" height="100%" colorInterpolationFilters="sRGB">
            <feFlood floodColor="#A8907E" floodOpacity="1" result="color"/>
            <feComposite in="color" in2="SourceAlpha" operator="in"/>
          </filter>

          {/* Soft glow for active muscle zones */}
        ...
        {/* Silhouette — recolored to warm light brown */}
        <image
          href="/images/pilates/body-model.png"
          x="0" y="0"
          width={VW} height={VH}
